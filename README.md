...
![Claude Window screenshot](docs/screenshot.png)
...
- [Claude Code CLI](https://docs.anthropic.com/en/docs/claude-code) installed and on `$PATH` (`npm install -g @anthropic-ai/claude-code`)
...
- **API key is stored in plaintext** at `~/.workbench/profile.json`. Keychain integration is planned.
- **Search** is not yet implemented.
- **Settings** — only Account and Appearance panes are functional. Other sections are coming.
- **CSP** is currently disabled in the WebView (`csp: null` in `tauri.conf.json`).
...
| File | Contents |
|---|---|
| `profile.json` | API key, project list, layout preferences |
